...
// const TAB_BAR_HEIGHT = 160; // Adjust this value based on your tab bar height
...
const {width: SCREEN_WIDTH, height: SCREEN_HEIGHT} = Dimensions.get('window');
const TAB_BAR_HEIGHT = 130; // Match this with BottomTabNavigator's barStyle height
...
const VideoFeed: React.FC<VideoFeedProps> = ({
  ...
  const doubleTapRef = useRef(false);
  const commentsListRef = useRef<FlatList>(null);
  const videoHeight = SCREEN_HEIGHT - TAB_BAR_HEIGHT; // Responsive height

  // Handle scroll direction for tab bar animation
  useEffect(() => {
    const listener = scrollY.addListener(({value}) => {
      const isScrollingUp = value < lastScrollY.current && value > 0;
      const isScrollingDown = value > lastScrollY.current;

      if (isScrollingUp) {
        animateTabBar(true); // Show tab bar
      } else if (isScrollingDown) {
        animateTabBar(false); // Hide tab bar
      }
    });

    return () => scrollY.removeListener(listener);
  }, [scrollY, lastScrollY, animateTabBar]);
